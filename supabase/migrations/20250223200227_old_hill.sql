/*
  # Implementação de Credenciais de API de Suporte
  1. Novas Tabelas
    - support_api_credentials: Armazena credenciais de API
    
  2. Funções
    - generate_support_id(): Gera ID único de 4 dígitos
    - generate_api_key(): Gera chave API segura
    - is_valid_cpf(): Valida CPF
    - is_valid_cnpj(): Valida CNPJ
    - is_valid_document(): Valida CPF ou CNPJ
    
  3. Triggers
    - Geração automática de ID e chave
...
BEGIN
  ...
  IF EXISTS (
    SELECT 1
    FROM pg_trigger
    WHERE tgname = 'support_credentials_before_insert'
  ) THEN
    -- Excluir o gatilho se ele existir
    EXECUTE 'DROP TRIGGER support_credentials_before_insert ON support_api_credentials';
  END IF;
END $$;
